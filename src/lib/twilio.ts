...
export async function notifyAdminNewRegistration(name: string, email: string) {
  return sendWhatsApp(process.env.ADMIN_WHATSAPP!, `🔔 ${APP_NAME}: New registration from ${name} (${email}). Review at /admin/members`)
}

export async function notifyAdminPayment(name: string, tier: string, amount: string) {
  return sendWhatsApp(process.env.ADMIN_WHATSAPP!, `💰 ${APP_NAME}: ${name} upgraded to ${tier} (${amount}).`)
}
